...
```powershell
C:\dlc_live_env\Scripts\python.exe -m pip install -r C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\requirements.txt
```
...
Запустите `C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\install_python_env.ps1`. Он:
...
- [ ] Шаг 2 (остальное): pip install -r C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\requirements.txt (БЕЗ --index-url)
...
### ARTIFACTS
- C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\requirements.txt — очищенный, дедуплицированный pip-устанавливаемый набор; строки torch закомментированы с пометкой про cu128 index-url; gxipy и полный deeplabcut явно исключены с пояснениями
- C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\install_python_env.ps1 — PowerShell-скрипт: проверяет Python 3.10, создаёт venv, ставит torch +cu128 отдельным шагом, затем requirements.txt с PyPI, гоняет pip check и smoke-тест torch.cuda/dlclive (синтаксис скрипта проверен Parser-ом — PARSE OK)
...
- **Все рабочие Gen3-файлы НЕ добавлены в git (untracked)**: `single_rt_dlc_live_bridge.py`, `dual_rt_dlc_live.py`, `config_dual_rt_dlc_live.py`, `live_profiles.py`, `run_live_profile.py`, `send_dual_dlc_bridge_test.py`, `check_online_buffering.py`, вся папка `optimization/`, `_migration/`, и все свежие `.md`-доки.
...
- `_migration/requirements.lock.txt` (57 пинов),
...
$dst = "C:\dlc\_migration_bundle\DLC_OBS_Spinal_cord_stimulation.zip"
New-Item -ItemType Directory -Force "C:\dlc\_migration_bundle" | Out-Null
...
2. Перенести также (отдельно): файл модели `.pt` (93 МБ), всю папку `C:\config_daheng\` (~124 КБ), `_migration\requirements.lock.txt` (уже внутри архива). Опционально — `C:\dlc\videos\` и/или полный проект 2.3 ГБ для дообучения.
...
### CHECKLIST
- [ ] На ИСХОДНОЙ машине упаковать рабочее дерево C:\dlc\DLC_OBS_Spinal_cord_stimulation в zip, ИСКЛЮЧИВ *.mp4, *.log, *_benchmark.csv, debug_snapshots/, __pycache__/, optimization/dump_*.csv, но ВКЛЮЧИВ все .py (особенно untracked Gen3: single_rt_dlc_live_bridge.py, dual_rt_dlc_live.py, config_dual_rt_dlc_live.py, live_profiles.py, run_live_profile.py, optimization/*.py), все config_*.py, _migration/requirements.lock.txt, .gitignore, .md-доки
...
```
C:\dlc_live_env\Scripts\python.exe C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\check_environment.py
```
...
```
powershell -ExecutionPolicy Bypass -File C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\check_environment.ps1
```
...
### CHECKLIST
- [ ] На целевом ПК скопировать рабочее дерево C:\dlc\DLC_OBS_Spinal_cord_stimulation целиком (НЕ git clone — git устарел), вместе с _migration\check_environment.py, check_environment.ps1 и requirements.lock.txt
...
- [ ] Запустить: C:\dlc_live_env\Scripts\python.exe _migration\check_environment.py (или check_environment.ps1)
...
### ARTIFACTS
- C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\check_environment.py — stdlib-only пре-флайт-проверка окружения (8 блоков: Python, pip-пины vs lock, torch/CUDA/GPU sm_120, nvidia-smi, Galaxy SDK+gxipy+GenTL env, модель .pt, конфиги камеры, боевые скрипты); печатает PASS/FAIL/TODO, exit 0/1
- C:\dlc\DLC_OBS_Spinal_cord_stimulation\_migration\check_environment.ps1 — тонкая PowerShell-обёртка: находит python (venv C:\dlc_live_env → $env:DLC_LIVE_PYTHON → python/py из PATH) и запускает check_environment.py, пробрасывая код возврата
