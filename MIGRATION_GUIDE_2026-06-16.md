...
- `_migration/requirements.lock.txt` — точный слепок (57 пакетов).
- `_migration/requirements.txt` — очищенный pip-устанавливаемый список (torch отдельно, см. ниже).
- `_migration/install_python_env.ps1` — создаёт venv и ставит всё.
- `_migration/check_environment.py` (+ `.ps1`) — **проверяет готовность целевого ПК** и печатает PASS/FAIL/MISSING. На рабочей машине даёт `46 OK / 0 MISSING / 0 MISMATCH`.
...
**Шаг 3 — venv + зависимости.** Запустить `_migration\install_python_env.ps1` (создаёт `C:\dlc_live_env`, ставит torch с cu128-индекса, затем `requirements.txt` с PyPI). Либо вручную:
```powershell
